...
data = read_smiles_property_file('./benchmark_datasets/HIV_updated.csv',
                                 cols_to_read=[0] + [2],
                                 keep_header=False)
smiles = data[0]
...
save_smiles_property_file('./benchmark_datasets/hiv/train.smi', X_train, y_train)
save_smiles_property_file('./benchmark_datasets/hiv/test.smi', X_test, y_test)

from openchem.data.smiles_data_layer import SmilesDataset
train_dataset = SmilesDataset('./benchmark_datasets/hiv/train.smi',
                              delimiter=',', cols_to_read=list(range(2)),
                              tokens=tokens, augment=True)
test_dataset = SmilesDataset('./benchmark_datasets/hiv/test.smi',
                            delimiter=',', cols_to_read=list(range(2)),
                            tokens=tokens)
predict_dataset = SmilesDataset('./benchmark_datasets/hiv/test.smi',
...
model_params = {
    ...
    'criterion': MultitaskLoss(ignore_index = 9, n_tasks=1).cuda(),
    ...
    'encoder_params': {
        ...
        'layer': "LSTM", # default LSTM
        'encoder_dim': 128, # default: 128
        'n_layers': 3, # default 4
        'dropout': 0.3, # default: 0.8                  best: 0.4 + mlp_dropout 0.4
    ...
    'mlp_params': {
        ...
        'hidden_size': [128, 1], # default [128, 12]
