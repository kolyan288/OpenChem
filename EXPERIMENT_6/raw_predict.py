...
class MyModel:
    ...
    def predict(self):
        ...
        self.pred = pd.read_csv('logs/tox21_rnn_log/predictions.txt', names = self.column_names)
        return self.pred
...
for i in range(N_TASKS):
    ...
    task = f'Task{i + 1}'
    predicted = np.array([pred1[task], pred2[task], pred3[task], pred4[task], pred5[task]]).mean(axis = 0)
