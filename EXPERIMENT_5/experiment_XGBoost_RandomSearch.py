...
def load_tox21(desc):
    ...
    if desc == 'MorganFP':
        ...
        if os.path.isfile(train_csv) and os.path.isfile(test_csv):
            ...
            scaler = MinMaxScaler()
            pca = PCA(n_components = 500, svd_solver = 'full', random_state = 123)
    ...
    return (X_train_SMILES, X_test_SMILES, y_train, y_test) 
    ...
    def fit(self, task):
        ...
        if self.task == 'multiclass_multilabel':
            ...
            for i in self.param_grid_XGBC2:
                ...
                self.model_mc_ml = MultiOutputClassifier(classifier).fit(self.X_train, self.y_train_mc_ml)
                self.y_pred_mc_ml = np.array(self.model_mc_ml.predict_proba(self.X_test))
            ...
            for i in range(12):
                ...
                self.model_mc_sl = search.fit(self.X_train, self.y_train_mc_sl[:, i])
                print(f'КЛАСС {i + 1}')
                print(self.model_mc_sl.best_params_)

                self.y_pred_mc_sl = self.model_mc_sl.predict_proba(self.X_test)
            ...
            for i in range(12):
                ...
                search = self.model_tc_sl.fit(self.X_train[ind1], self.y_train[ind1, i])
                ...
                y_pred = search.predict(self.X_test[ind2])
        ...
        if task not in ['multiclass_multilabel', 'twoclass_multilabel', 'twoclass_singlelabel', 'multiclass_singlelabel']:
            ...
            if self.task == 'multiclass_multilabel':
                ...
                for i in range(12):
                    ...
                    search = self.model_mc_sl.fit(self.X_train, self.y_train_mc_sl[:, i])
                    print(f'КЛАСС {i + 1}')
                    print(search.best_params_)

                    self.y_pred_mc_sl = search.predict_proba(self.X_test)
                ...
                self.model_tc_ml = GridSearchCV(classifier, self.param_grid_CatBoost, scoring = 'f1', verbose = 2)
                search = self.model_tc_ml.fit(self.X_train, np.round(self.y_train_tc_ml))
                ...
                self.y_pred_tc_ml = np.array(search.predict(self.X_test))
...
data = load_tox21(desc = 'MordredFP')

# task1 = MyXGBoost(data) 
# task1.fit('multiclass_multilabel')
# task1.fit('multiclass_singlelabel')
# task1.fit('twoclass_multilabel')
# task1.fit('twoclass_singlelabel')

task2 = MyCatBoost(data)
# task2.fit('multiclass_multilabel')
# task2.fit('multiclass_singlelabel')
task2.fit('twoclass_multilabel')
# task2.fit('twoclass_singlelabel')
